#include "stdafx.h"
#include "MonsterScript_Baphomet.h"
...
CMonsterScript_Baphomet::CMonsterScript_Baphomet()
{
}
...
void CMonsterScript_Baphomet::start()
{
	...
	m_pTransform->SetLocalScale(Vec3(300.f, 300.f, 1.f));
	m_vPrePos = m_vCurPos;
	...
	pStandingState->SetAnimationName(L"UpStanding_Baphomet", L"DownStanding_Baphomet");
	...
	pMoveState->SetAnimationName(L"UpWalking_Baphomet", L"DownWalking_Baphomet");
	pState = m_pStateMgr->AddState(L"MOVE", pMoveState);
	if (pState)
		pState->InsertNextKey(setStandardKey);
	// DIE
	CPoringDieState* pDieState = new CPoringDieState(this);
	pDieState->SetAnimationName(L"UpDying_Baphomet", L"DownDying_Baphomet");
	pState = m_pStateMgr->AddState(L"DIE", pDieState);
	if (pState)
		pState->InsertNextKey(setStandardKey);
	// Attack
	CPoringAttackState* pAttackState = new CPoringAttackState(this);
	pAttackState->SetAnimationName(L"UpAttack_Baphomet", L"DownAttack_Baphomet");
	pState = m_pStateMgr->AddState(L"ATTACK", pAttackState);
	if (pState)
		pState->InsertNextKey(setStandardKey);

	Animator()->PlayAnimation(L"DownStanding_Baphomet");
	m_pStateMgr->ChangeState(L"STANDING");
}

int CMonsterScript_Baphomet::update()
{
	float fDT = DT();
	m_fSurvivalTime += fDT;
	m_vPrePos = m_vCurPos;
...
void CMonsterScript_Baphomet::KeyInput()
{
}

void CMonsterScript_Baphomet::StateExecute()
{
...
void CMonsterScript_Baphomet::Reset()
{
	CMonsterScript::Reset();
	tStatus* pStatus = CMonsterTable::GetThis()->FindMonsterStatus(L"Baphomet");
	if (NULL != pStatus)
	{
		m_tStatus = *pStatus;
	}
...
void CMonsterScript_Baphomet::SoundEffectPlay(const int _idx)
{
...
void CMonsterScript_Baphomet::GetMessageData(CMessage * _pData)
{
