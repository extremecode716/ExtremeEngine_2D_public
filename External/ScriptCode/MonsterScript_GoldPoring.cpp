#include "stdafx.h"
#include "MonsterScript_GoldPoring.h"
...
CMonsterScript_GoldPoring::CMonsterScript_GoldPoring()
{
}
...
void CMonsterScript_GoldPoring::start()
{
	...
	// Stat
	tStatus* pStatus = CMonsterTable::GetThis()->FindMonsterStatus(L"GoldPoring");
	if (NULL != pStatus)
	{
		m_tStatus = *pStatus;
		m_iMinDamage = 50;
	...
	m_pCollq_Monster = CMessageColleaque::Create(m_pMediator, m_strCollqName, this); // Monster_Poring
	...
	pStandingState->SetAnimationName(L"UpStanding_GoldPoring", L"DownStanding_GoldPoring");
	...
	pMoveState->SetAnimationName(L"UpWalk_GoldPoring", L"DownWalk_GoldPoring");
	pState = m_pStateMgr->AddState(L"MOVE", pMoveState);
	if (pState)
		pState->InsertNextKey(setStandardKey);
	// DIE
	CPoringDieState* pDieState = new CPoringDieState(this);
	pDieState->SetAnimationName(L"UpDying_GoldPoring", L"DownDying_GoldPoring");
	pState = m_pStateMgr->AddState(L"DIE", pDieState);
	if (pState)
		pState->InsertNextKey(setStandardKey);
	// Attack
	CPoringAttackState* pAttackState = new CPoringAttackState(this);
	pAttackState->SetAnimationName(L"UpWalk_GoldPoring", L"DownWalk_GoldPoring");
	pState = m_pStateMgr->AddState(L"ATTACK", pAttackState);
	if (pState)
		pState->InsertNextKey(setStandardKey);

	Animator()->PlayAnimation(L"DownStanding_GoldPoring");
	m_pStateMgr->ChangeState(L"STANDING");
}

int CMonsterScript_GoldPoring::update()
{
	float fDT = DT();
	m_fSurvivalTime += fDT;
	m_vPrePos = m_vCurPos;
...
void CMonsterScript_GoldPoring::KeyInput()
{
}

void CMonsterScript_GoldPoring::StateExecute()
{
...
void CMonsterScript_GoldPoring::Reset()
{
...
void CMonsterScript_GoldPoring::SoundEffectPlay(const int _idx)
{
...
void CMonsterScript_GoldPoring::GetMessageData(CMessage * _pData)
{
